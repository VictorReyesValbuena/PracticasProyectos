...
Proyecto final del módulo de **Deep Learning & Computer Vision**. El objetivo es predecir la condición médica de un paciente a partir de dos fuentes de información complementarias —una **imagen dermatoscópica** y unos **datos tabulares clínicos**— empleando redes neuronales profundas y estrategias de **fusión multimodal**.
...
- Imputación de valores nulos en `age` con la mediana y normalización al rango `[0, 1]`.
- Codificación *one-hot* de `sex` y `localization` → vector tabular de 19 dimensiones.
- Reshape de las imágenes aplanadas a tensores `(N, 28, 28, 3)` en formato `channels_last`.
...
### Modelo 1D (tabular)
Red densa con API funcional: 4 capas ocultas de 64 neuronas, con activaciones ReLU/SELU y softmax final de 7 clases. Se nombra explícitamente la capa `tabular_features` para su reutilización en early-fusion.

### Modelo 2D (imágenes)
Se implementan y comparan **cuatro arquitecturas**:
- **CNN propia**: dos bloques Conv2D + BatchNorm + ReLU, Global Max Pooling y cabeza densa.
...
- Se concatenan sus **probabilidades softmax** (7 + 7 = 14 dimensiones).
...
- Se extraen las **representaciones intermedias** (features) de cada sub-modelo mediante `get_layer('tabular_features').output` y análogos en visión.
- Se concatenan los vectores de features (64 + 1024 = 1088 dimensiones).
